...
void CDataSMD::Render( VOID* p )
{
	if ( Loaded )
	{
		...
		while ( pMesh != NULL )
		{
			glBindTexture(GL_TEXTURE_2D, tex[pMesh->texture]);
...
void CDataSMD::RenderBones( VOID* p )
{
...
bool CDataSMD::Load( char* filename )
{
	if ( filename != NULL)
	{
		...
		m_pModel = new smd_model;					DBG_MESSAGE("alloc memory\n");
		ZeroMemory( m_pModel, sizeof(smd_model) );	DBG_MESSAGE("zero memory\n");
		
		DBG_MESSAGE("Loading model...\n");
		LoadSMD( filename, m_pModel );
		DBG_MESSAGE("Load Succeeded\n");
		Loaded = true;
	}
	else
	{
		MessageBox(NULL, "WRONG FILENAME", "ERROR :", MB_OK | MB_ICONERROR);
	}
	return true;
}

bool CDataSMD::Unload()
{
	if ( m_pModel != NULL )
	{
		smd_mesh* pMesh = m_pModel->start_mesh;
...
//////////////////////////////////////////////////////////////////////////
int CDataSMD::LoadSMD( char* path, smd_model* pModel )
{
	FILE* pfile;
	char cmd[1024];
	int option;
...
//		- int (Error message, result of the function)
//////////////////////////////////////////////////////////////////////////
int CDataSMD::LoadNodes( FILE* pfile, smd_model* pModel )
{
	int i = 0;
...
//		- int (Error message, result of the function)
//////////////////////////////////////////////////////////////////////////
int CDataSMD::LoadSkeleton( FILE* pfile, smd_model* pModel )
{
	int i = 0;
...
//		- int (Error message, result of the function)
//////////////////////////////////////////////////////////////////////////
int CDataSMD::LoadTriangles( FILE* pfile, smd_model* pModel )
{
	smd_mesh* pMesh;
	int i = 0;
	int k = 0;
...
//		- int (Error message, result of the function)
//////////////////////////////////////////////////////////////////////////
int CDataSMD::AddNode( char* input, smd_model* pModel )
{
	int index;
	int parent;
	char name[64];
...
//		- int (Error message, result of the function)
//////////////////////////////////////////////////////////////////////////
int CDataSMD::AddBone( char* input, smd_model* pModel )
{
	int index;
	vec3_f pos, rot;
	smd_bone* pbone;
...
//		- int (Error message, result of the function)
//////////////////////////////////////////////////////////////////////////
int CDataSMD::AddTriangle( smd_mesh* pmesh, smd_triangle* ptriangle )
{
	if ( !pmesh )
		return -1;
	if ( !ptriangle )
...
//////////////////////////////////////////////////////////////////////////
smd_mesh* CDataSMD::GetMesh( char* texture, smd_model* pModel )
{
	...
	char tmp[MAX_PATH];
	sprintf(tmp, "%s/%s", GetTexturePath(ptexture->name), ptexture->name );
	int index = LoadGLTexture(tmp);
	if ( index != -1 )
	{
		ptexture->index = index;
	}

	// create new mesh
	pmesh->texture = pModel->numtextures;
...
//		- char* (Striped string)
//////////////////////////////////////////////////////////////////////////
char* CDataSMD::StripString( char* str )
{
	char* res;
...
//		- 
//////////////////////////////////////////////////////////////////////////
int CDataSMD::LoadGLTexture(char* filename)					// Load Bitmaps And Convert To Textures
{
	//static int tex_num = -1;
    ...
    if (data != nullptr)				// Load Texture
    {
		glGenTextures(2, &tex[tex_num]);					// Create The Texture
...
//		- 
//////////////////////////////////////////////////////////////////////////
char* CDataSMD::GetTexturePath(char* texture)
{
	for( int i = strlen(filename); i > 0; i--)
	{
		if ( filename[i] == '\\' || filename[i] == '/' )
		{
			...
			ZeroMemory(tmp, sizeof(char)*i+1);
			memcpy(tmp, filename, sizeof(char)*i);
			return tmp;
		}
	}
	return "";
